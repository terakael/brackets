(function() {
    ...
    TradeWindow.prototype.update = function(obj) {        
        ...
        for (let slot in obj.playerTradeData) {
            ...
            let currentRow = ~~(slot / 5);// four items per row
            let currentColumn = slot % 5;
            playerSlot.setLocalPosition(this.playerRect.left + 20 + (currentColumn * playerSlot.rect.width), this.playerRect.top + 30 + (currentRow * playerSlot.rect.height));
        ...
        for (let slot in obj.otherTradeData) {
            ...
            let currentRow = ~~(slot / 5);// four items per row
            let currentColumn = slot % 5;
            otherSlot.setLocalPosition(this.otherRect.left + 20 + (currentColumn * otherSlot.rect.width), this.otherRect.top + 30 + (currentRow * otherSlot.rect.height));
